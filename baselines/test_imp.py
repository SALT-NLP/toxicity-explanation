...
GPT_DICT = {
    'TRAINED MODEL': 'model/gpt_implicit_hate',
    'BASE MODEL': 'openai-gpt',
}

GPT2_IMPLICIT_DICT = {
    'TRAINED MODEL': 'model/gpt2_implicit_hate',
    'BASE MODEL': 'gpt2',
}
...
def parse_args():
  ...
  parser.add_argument('--data_file', type=str, default='../data/implicit_hate_v1_stg3_posts.tst.tsv', help='Data File to load.')
...
def get_active_dict(args):
  if args.model_type == 'gpt':
    active_dict = GPT_IMPLICIT_DICT
  else:
    active_dict = GPT2_IMPLICIT_DICT
...
def check_args(args):
  ...
  results_file = 'results/' + args.model_type + '_' + data_source + '_imp.csv'

  active_dict = get_active_dict(args)
  active_dict['TO ACTUAL'] = pred_file_name + '_imp_actual.pickle'
  active_dict['TO PRED'] = pred_file_name + '_imp_pred.pickle'
...
if __name__ == "__main__":
  ...
  if args.predict:
    ...
    actual = get_samples_from_actual(df, PRED_COL, post_ids=hitid_set, impl=True)
